...
| `claude "查询"` | 启动带初始提示的 REPL | `claude "explain this project"` |
| `claude -p "查询"` | 打印模式 - 查询后退出 | `claude -p "explain this function"` |
| `cat file \| claude -p "查询"` | 处理管道内容 | `cat logs.txt \| claude -p "explain"` |
| `claude -c` | 继续最近的对话 | `claude -c` |
| `claude -c -p "查询"` | 以打印模式继续 | `claude -c -p "check for type errors"` |
| `claude -r "<会话>" "查询"` | 按 ID 或名称恢复会话 | `claude -r "auth-refactor" "finish this PR"` |
...
| `claude auth status` | 检查认证状态（登录返回 exit 0，未登录返回 exit 1） | `claude auth status` |
...
| `--remote "任务"` | 在 claude.ai 上创建 Web 会话 | `claude --remote "implement API"` |
...
| `--fallback-model` | 过载时自动切换到备用模型 | `claude -p --fallback-model sonnet "query"` |
...
**`--system-prompt-file` 仅在打印模式下使用。对于交互模式，使用 `--system-prompt` 或 `--append-system-prompt`。**
...
`--agents` 标志接受一个 JSON 对象，定义会话的自定义子代理。
...
将 Claude 作为可编程 API 与 `jq` 解析一起使用。
...
提示中的 "ultrathink" 关键字激活深度推理。`max` 努力级别仅为 Opus 4.6 专属。
...
| 带安全性的自主运行 | `claude --enable-auto-mode --permission-mode auto` |
...
- 使用 `--json-schema` 强制结构
- 在提示中添加明确的 JSON 指令
- 使用 `--output-format json`（而不仅仅是在提示中请求 JSON）
...
- 为自动化使用 `--dangerously-skip-permissions`（谨慎使用）
